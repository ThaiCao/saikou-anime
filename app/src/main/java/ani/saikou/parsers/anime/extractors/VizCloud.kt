...
class VizCloud(override val server: VideoServer) : VideoExtractor() {
    ...
    private suspend fun encrypt(text:String, steps:List<Char>): String {
        ...
        for(step in steps){
            when(step){
                'o'->{output=NineAnime.encrypt(output,getKey().encryptKey).replace("/", "_")}
                's'->{output=s(output)}
                'a'->{output=output.reversed()}
            }
        }
        return output
    }
    ...
    companion object {
        ...
        suspend fun getKey(): CipherKey {
            cipherKey = if (cipherKey != null && (lastChecked - System.currentTimeMillis()) < 1000 * 60 * 30) cipherKey!!
                ...
                Mapper.parse<CipherKey>(Base64.decode(Base64.decode(Base64.decode(client.get(jsonLink).text, Base64.NO_WRAP),Base64.NO_WRAP),Base64.NO_WRAP).decodeToString())
            }
            return cipherKey!!
        }
        ...
        suspend fun dashify(input:String): String {
            ...
            val mapped = input.mapIndexedNotNull { i, c ->
                ...
                when(operation.first()){
                    ...
                    "<<" -> c.code shl operand
                    "^"  -> c.code xor operand
                    else -> throw Exception("Unknown operator needed to be implemented")
                }
            }.joinToString("-")
            return mapped
        }
